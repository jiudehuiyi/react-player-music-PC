...
import {formatAlbumNewComment,formatHotCommentPublishTime} from '../../../api/formatData';
import './index.scss';
 class NewComment extends Component {
  ...
  render() {
    //   console.log(this.props.data)
      const newCommentData = this.props.data;
      const needCommentDate = formatAlbumNewComment(this.props.data);
        // console.log(needCommentDate)
      //渲染最新评论
      const renderNewComment = needCommentDate.comments && needCommentDate.comments.length>0?needCommentDate.comments.map( (item,index,arr)=>{
    ...
    return (
      <div>
        {
            newCommentData.comments&&newCommentData.comments.length>0?
            <div className='newComment' >
                <div className='newComment-title' style={{ borderBottom:"1px dashed #CFCFCF",paddingBottom:"10px" }}>
                    ...
                    <span style={{ color:"#000",fontSize:"12px",fontWeight:"bold" }}>({needCommentDate.total})</span>
                </div>
                {
                    renderNewComment
                }
                <div style={{ marginTop:"40px" }}>
                     <Row>
                        <Col span={4}></Col>
                        <Pagination
                         total={needCommentDate.total} defaultPageSize={20} 
